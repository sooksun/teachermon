...
export default function MentoringEditPage() {
  ...
  useEffect(() => {
    if (visit) {
      setFormData({
        ...
        visitType: visit.visitType || 'LESSON_STUDY',
        observer: visit.observer || '',
        focusArea: visit.focusArea || 'CLASSROOM',
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: Record<string, unknown>) => {
      const response = await apiClient.put(`/mentoring/${id}`, data);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['mentoring-visit', id] });
    ...
    <MainLayout>
      <div className="space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">แก้ไขการลงพื้นที่</h1>
